...
The compose service uses a project-local `./tunnel:/var/cloudflared` mount, exposes `UUID` in the container environment, mounts the startup wrapper from `${cloudflare_DDDC}/startup` to `/opt/startup`, copies the runtime assets into `/etc/cloudflared`, and then runs `cloudflared` with `--config /etc/cloudflared/config.yml`.
